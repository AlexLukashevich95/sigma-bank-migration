    ...
    public static Config parse(String[] args) {
        ...
        for (String arg : args) {
            ...
            if (a.startsWith("--sort=") || a.startsWith("-s=")) {
                ...
                if (val.equals("name")) sortBy = SortBy.NAME;
                else if (val.equals("salary")) sortBy = SortBy.SALARY;
                else throw new IllegalArgumentException("Неизвестный тип сортировки: " + val);
                continue;
            }
            if (a.startsWith("--order=")) {
                String val = a.substring("--order=".length()).trim();
                if (val.equals("asc")) order = Order.ASC;
                else if (val.equals("desc")) order = Order.DESC;
                else throw new IllegalArgumentException("Неизвестный порядок сортировки: " + val);
                continue;
            }
            if (a.equals("--stat")) {
                stat = true;
            ...
            if (a.startsWith("--output=") || a.startsWith("-o=")) {
                String val = a.substring(a.indexOf('=') + 1).trim();
                if (val.equals("console")) output = Output.CONSOLE;
                else if (val.equals("file")) output = Output.FILE;
                else throw new IllegalArgumentException("Неизвестный способ вывода статистики: " + val);
                continue;
            }
        ...
        if (!stat) {
            ...
            if (output == Output.FILE && (path == null || path.isEmpty())) {
                throw new IllegalArgumentException("--output=file требует указания --path=<путь к выходному файлу>.");
            }
